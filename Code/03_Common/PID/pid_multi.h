 ...
 * @details
 * 1. 文件作用：声明外环/内环可选算法的串级 PID 数据结构与计算接口。
 * 2. 解耦边界：仅提供控制算法组合框架，不依赖电机、传感器等硬件模块。
 * 3. 上层绑定：`DccTask`、`StepperTask` 等控制任务可按场景配置单环或串级模式。
 * 4. 生命周期：调用方负责对象初始化与参数更新，运行期周期调用 compute 接口。
 */
