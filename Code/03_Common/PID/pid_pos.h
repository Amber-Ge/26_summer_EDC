 ...
 * @details
 * 1. 文件作用：声明位置式 PID 对象、参数与计算接口。
 * 2. 解耦边界：仅提供控制算法，不绑定具体硬件执行器。
 * 3. 上层绑定：控制任务可用于位置/角度/偏差等慢变量闭环。
 * 4. 内置保护：包含积分限幅与输出限幅，降低积分饱和风险。
 */
...
/**
 * @brief 位置式 PID 默认输出上限（未在外部覆盖时生效）。
 */
...
/**
 * @brief 位置式 PID 默认积分上限（未在外部覆盖时生效）。
 */
 ...
 * @param ki 积分系数。
 * @param kd 微分系数。
 * @param out_max 输出上限（下限默认 `-out_max`）。
 * @param integral_max 积分上限（下限默认 `-integral_max`）。
 */
