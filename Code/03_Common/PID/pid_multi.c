 ...
 * @brief   多环 PID 组合实现。
 * @details
 * 1. 文件作用：实现串级 PID 初始化、模式切换与双环计算流程。
 * 2. 上下层绑定：上层由任务层周期调用；下层复用 `pid_pos` 与 `pid_inc`。
 */
#include "pid_multi.h"
#include <stddef.h>

/**
 * @brief 默认参数区（允许在 `pid_config.h` 中通过同名宏覆盖）。
 */
...
{
    float result = value; // result：限幅结果，默认等于输入。
...
/**
 * @brief 根据算法类型运行单个 PID 环。
 * @param algo 算法类型。
 * @param pos 位置式 PID 对象指针。
 * @param inc 增量式 PID 对象指针。
 * @param target 目标值。
 * @param measure 测量值。
 * @return float 当前环输出值。
 */
...
{
    float output = 0.0f; // output：当前环输出。
...
{
    float temp = 0.0f; // temp：上下限交换临时变量。
...
{
    float outer_output = 0.0f; // outer_output：外环输出（串级时作为内环目标）。
    float final_output = 0.0f; // final_output：最终控制输出。
